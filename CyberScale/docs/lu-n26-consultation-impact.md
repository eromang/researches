...
> **Note, 2026-08-06.** The counts above were taken from `contextual_train.csv`, the **v3** corpus; the deployed model is trained on **v4**. The distinction barely moves them — v4 carries the same 59-type vocabulary, with counts within a few percent (e.g. `trust_service_provider` 137 → 126) — so the analysis stands as written. `contextual_train.csv` has since been removed; v4 is byte-reproducible from published inputs.
>
> **What did not stand is the "coverage is thin, not a blocker" reading.** Measured directly against the deployed model on the v4 test split, the three re-routed types score **66.0 %** (33/50) against **81.7 %** overall. The Wilson 95 % intervals — [52.2, 77.6] and [80.6, 82.8] — do not overlap, so the gap is established despite the small n. Per type: `ixp_operator` 63.2 % (n=19), `public_ecn_provider` 60.0 % (n=15), `public_ecs_provider` 75.0 % (n=16). This answers open question Q4 in the affirmative: the IR scope fix moved these three from a wrong deterministic answer to a measurably weaker probabilistic one. Reproduce with `evaluation/eval_contextual_vocabulary.py --by-entity-type`.
>
> *(An intermediate correction here claimed the deployed model was v2-era and had never seen these types. That was wrong — it rested on a `git diff` over gitignored paths. See `data/training/contextual/README.md`.)*
