...
| D6 | `evaluation/benchmark_curated.py` is not reproducible | P2 | OPEN | *Found closing D1.* Three consecutive runs on identical data gave T 91.3% / 93.5% / 91.3% and matrix 89.1% / 91.3% / 89.1%. O is stable at 82.6% because it is rules-based. The T-model retains MC dropout, so run-to-run comparison of this benchmark is meaningless without seeding or averaging. It also still loads the **deprecated** ML T-model from `data/models/technical` rather than the deterministic `derive_t_level()` that replaced it in v4. |
