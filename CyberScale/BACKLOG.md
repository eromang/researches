...
| D2 | Decide the fate of `contextual_train.csv` | P3 | **DONE** 2026-08-05 | **Deleted — but the premise was wrong.** The entry said "delete if the HuggingFace v4 dataset is sufficient provenance". There is no v4 on HuggingFace: `eromang/cyberscale-contextual-training` serves **v2** under the name `contextual_training.csv` (sha256 `d3ff30f8…`, byte-identical to the local `contextual_training_v2.csv`, uploaded 2026-03-31, before v3 and v4 existed). What actually justifies deletion is stronger: v4 is **byte-reproducible** from published + tracked inputs — regenerated and hash-matched to `090bd96c…` from `training_cves.csv` (published byte-for-byte on HF), tracked `sector_severity_rules.json` and tracked `contextual_cls.json`, generation being seeded. v3 was the input to a retrain **reverted** in `575c625`, so it is provenance for an artifact never shipped. It stays in git history at `106ad93`; removal frees working-tree bytes only, not clone size. |
| D8 | Deployed contextual model has zero entity-vocabulary overlap with production | P1 | OPEN | *Found closing D2.* `data/models/contextual/` is byte-identical to its pre-v3 state — the v3 retrain was reverted in `575c625` (empty commit body) and nothing reverted the **code**. So the shipped model is trained on v2's 8 entity types (`msp, utility, bank, cloud_provider, hospital, individual, sme, government`) while `models/contextual.py` validates against **59** and raises on anything else. **Intersection: 0.** Inference also emits `ms_established:` / `ms_affected:`, present in 0 of the 32,000 v2 training rows. The advertised 81.7 % accuracy / 81.5 % macro F1 were measured on v2-shaped inputs. This path is production-reachable via `tools/contextual.py` and `tools/entity_incident.py`. **Impact not yet characterised:** a 5-scenario probe varying entity_type over all 59 changed the answer in 1 of 5 (S3: 45 Critical / 14 High); the other 4 were invariant, as were the 8 trained types. So the skew is real but 5 scenarios do not show it to be severe, and do not show it harmless either. Fix is a retrain on v4 — a maintainer call, not a cleanup. |
