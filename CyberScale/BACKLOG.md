...
| D3 | Three IR re-routed entity types underperform, measurably | P1 | OPEN | *Was "thin training coverage"; upgraded to P1 on measurement 2026-08-06.* `ixp_operator`, `public_ecn_provider` and `public_ecs_provider` score **66.0 % (33/50)** on the v4 test split against **81.7 %** overall. Wilson 95 % intervals [52.2, 77.6] vs [80.6, 82.8] **do not overlap** — the gap is established despite the small n. Per type: 63.2 % (n=19), 60.0 % (n=15), 75.0 % (n=16). The doc's earlier reading — thin coverage, "not a blocker" — is superseded: this is not a coverage worry, it is a measured deficit on exactly the types the IR scope fix made the model answerable for. Fix is a retrain with those three topped up; `evaluation/eval_contextual_vocabulary.py --by-entity-type` is the before/after gate. |
