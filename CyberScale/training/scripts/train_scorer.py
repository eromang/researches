"""Train CyberScale severity classifier (ModernBERT 4-class classification).
...
class CVEDataset(Dataset):
    """Tokenised CVE descriptions with integer class labels."""
...
def train(
    ...
    # Drop rows with missing descriptions or scores
...
# ---------------------------------------------------------------------------
def main() -> None:
    parser = argparse.ArgumentParser(description="Train CyberScale severity classifier")
    parser.add_argument(
        "--data",
        type=str,
        required=True,
        help="Path to training CSV (cve_id, description, cvss_score, ...)",
    )
    parser.add_argument(
        "--config",
        type=str,
        required=True,
        help="Path to scorer_cls.json config",
    )
    parser.add_argument(
        "--output",
        type=str,
