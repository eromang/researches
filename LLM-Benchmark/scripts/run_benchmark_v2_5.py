#!/usr/bin/env python3
"""
EU Cyber Threat Landscape Benchmark Runner (Ollama HTTP API) - v2.5

v2.5 builds on v2.4 and adds:
- Optional /api/chat endpoint via --use-chat (default: /api/generate for backward compat)
- Optional --num-gpu to control GPU layer offloading
- Optional --strip-thinking to remove <think>...</think> tokens (e.g. DeepSeek-R1)
- Optional --no-think to append /no_think to prompts (disables thinking in qwen3-style models)
- Batched fsync every 10 records instead of per-record (final + interrupt still flush)
- New JSONL fields (all additive/optional): api_endpoint, strip_thinking, no_think, thinking_text, thinking_len_chars
...
Typical usage:
  ...
  python3 run_benchmark_ollama_api_v2_5.py \
...
- v2.5 flushes JSONL to disk every 10 records (and on final record / interrupt).
- --cooldown-auto scales cooldown by model size class; requires --cooldown-base > 0 to have any effect.
...
_CHAT_SYSTEM_MSG = """You are an EU-focused Cyber Threat Landscape Analyst.
You must NOT provide offensive instructions, exploit steps, malware development, or procedural attack detail.
Stay defensive, strategic, and evidence-based.
Do not fabricate CVEs, campaign names, actor names, dates, or statistics. If uncertain, say so.
...
def strip_thinking_tokens(text: str) -> Tuple[str, str]:
    """Remove <think>...</think> blocks from text.

    Returns (clean_text, thinking_text) where thinking_text is the
    concatenated content of all <think> blocks (without the tags).
    """
    if not text:
        return "", ""
    thinking_parts: List[str] = []
    for m in _THINK_RE.finditer(text):
        inner = m.group(0)[7:-8]  # strip <think> and </think>
...
def ollama_chat(
    ...
    """Call Ollama /api/chat with system + user message split."""
    ...
    return {"ok": True, "error": None, "response": response_text, "latency_ms": int((end - start) * 1000), "raw": out}
...
def generate_derived_outputs(
    ...
    for r in records:
        ...
        flat = {
            ...
            # v2.5 additive fields (None/empty for old records)
            "api_endpoint": r.get("api_endpoint", ""),
            "strip_thinking": r.get("strip_thinking", ""),
            "thinking_len_chars": r.get("thinking_len_chars", ""),
        }
...
def main() -> int:
    ...
    ap.add_argument("--keep-alive", default="30m", help="Ollama keep_alive, e.g., 10m, 30m, 1h, -1 (forever)")
    ...
    ap.add_argument("--cooldown-auto", action="store_true", help="Scale cooldown by estimated model size class (requires --cooldown-base > 0 to have any effect)")
    ...
    # v2.5 new options
    ap.add_argument("--use-chat", action="store_true", help="Use /api/chat endpoint instead of /api/generate (system+user message split)")
    ap.add_argument("--num-gpu", type=int, default=None, help="Number of GPU layers to offload (None = Ollama default)")
    ap.add_argument("--strip-thinking", action="store_true", help="Strip <think>...</think> tokens from output (e.g. DeepSeek-R1)")
    ap.add_argument("--no-think", action="store_true", help="Append /no_think to each prompt (disables thinking in qwen3-style models)")

    args = ap.parse_args()

    # Normalize keep_alive: integer values (e.g. -1, 0) must be sent as int, not string
    try:
        args.keep_alive = int(args.keep_alive)
    except ValueError:
        pass  # duration string like "30m" — keep as-is
    ...
    with open(jsonl_path, "a", encoding="utf-8") as jf:
                ...
                for temp in args.temps:
                    for rep in range(1, args.reps + 1):
                        for p in model_prompts:
                            ...
                            # Append /no_think suffix if requested (e.g. qwen3 non-thinking mode)
                            if args.no_think:
                            ...
                            if args.use_chat:
                                # /api/chat: split into system + user
                                if args.no_wrapper:
                                    system_msg = ""
                                    user_msg = prompt_text
                                else:
                                    system_msg = _CHAT_SYSTEM_MSG
                                    user_msg = prompt_text
                                res = ollama_chat(
                                ...
                                # /api/generate: existing behavior
                            ...
                            if args.strip_thinking:
                                out_text, thinking_text = strip_thinking_tokens(out_text)
                            ...
                            # Add thinking fields only when strip_thinking is enabled
                            if args.strip_thinking:
                                rec["thinking_text"] = thinking_text
                                rec["thinking_len_chars"] = thinking_len

                            jf.write(json.dumps(rec, ensure_ascii=False) + "\n")
                            jf.flush()
                            records_since_fsync += 1

                            # Batch fsync every FSYNC_BATCH records
                            if records_since_fsync >= FSYNC_BATCH:
                                os.fsync(jf.fileno())
                                records_since_fsync = 0
                            ...
                            extra = ""
                            if args.strip_thinking and thinking_len > 0:
                                extra = f" (thinking: {thinking_len} chars stripped)"
                            print(f"[{status}] {model} T={temp} rep={rep} {prompt_id} -> {rec['output_len_chars']} chars, {rec['latency_ms']} ms{extra}")
        ...
        # Final fsync for any remaining unflushed records
        if records_since_fsync > 0:
            jf.flush()
            os.fsync(jf.fileno())
